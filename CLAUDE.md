...
1. `filter_data` — date/account/amount/channel scoping. `window_days` is
   relative to the dataset max; `date_from`/`date_to` take explicit calendar
   dates. An explicit range must never be converted into a relative window.
2. `profile_data` — quick EDA summary (only when the query asks for overview)
2b. `aggregate_data` — plain per-account counting inside an amount band, with
   NO risk judgement. For factual count questions ("which customers made 10+
   transactions under $10,000"); the typology rules answer a different
   question and would return the wrong set.
