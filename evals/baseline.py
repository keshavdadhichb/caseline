...
METRICS_OUT = Path(__file__).resolve().parents[1] / "data" / "method_metrics.json"
...
def main() -> None:
    ...
    # The UI's "Method & performance" panel reads these exact numbers rather
    # than hardcoding its own — so the panel can never drift from what this
    # script actually measured. Regenerated by `make eval`; committed so the
    # demo serves real, reproducible figures with no compute at request time.
    payload = {
        "_comment": "Generated by evals/baseline.py on the held-out TEST split. Do not hand-edit.",
        "split": "test",
        "ground_truth_accounts": len(truth),
        "universe_accounts": len(universe),
        "global": {"baseline": baseline_metrics, "caseline": agent_metrics},
        "tiers": tiers,
        "precision_at_n": {str(n): precision_at_n(records, truth, n) for n in (50, 100)},
        "patterns": pattern_result,
        "ring": {"flagged": len(ring_flagged), "total": len(ring_accounts),
                 "aggregator_caught": "4521" in ring_flagged},
        "operational": vol,
    }
    METRICS_OUT.parent.mkdir(parents=True, exist_ok=True)
    METRICS_OUT.write_text(json.dumps(payload, indent=2, default=float))
    print(f"\nwrote machine-readable metrics -> {METRICS_OUT}")
