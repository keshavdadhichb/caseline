...
# moving the accumulated money onward, is the actual crime — the threshold
# evasion alone is what a busy, entirely legitimate cash business also does
# every week (a corner store depositing $9,200 most days is not laundering
# money; it just does a lot of cash business). What separates the two is
# what happens to the money AFTER it accumulates: a legitimate business
# leaves the funds in the account: a smurfing aggregator consolidates them
# out again quickly. So this rule now requires BOTH legs, scoped to the
# RECEIVING side specifically (the account accumulating the deposits) —
# the "leaves funds in place vs. moves them on" distinction is inherently
# about what an account does with money it RECEIVES, not what it sends.
# (A mule who only ever SENDS 5 sub-threshold payments and does nothing
# further is still worth reviewing, but "sends money in small pieces" is a
# weaker, more common pattern than "gathers many small deposits and moves
# them out"; the graph layer's FAN_IN_RING is the tool built to catch a
# gathering ring's individual senders — see METHODOLOGY.md's "what got
# worse" section for the direct consequence of this scope narrowing.)
STRUCTURING_MIN_COUNT = 5  # was 3 — a single busy week can produce 3-4 sub-threshold deposits innocently
STRUCTURING_WINDOW_DAYS = 7
NEAR_THRESHOLD_LOW = 9_500.0  # was 9,000 (10% band) — tightened to 5% below the threshold
...
def _structuring(df: pd.DataFrame) -> list[Flag]:
    ...
    near = recv[(recv.amount >= NEAR_THRESHOLD_LOW) & (recv.amount < NEAR_THRESHOLD_HIGH)]
    if near.empty:
        return []
    ...
    quick_candidates = set(near_counts[near_counts >= STRUCTURING_MIN_COUNT].index)
    ...
    for account_id, group in near.groupby("account_id"):
        ...
        for hi in range(len(times)):
            ...
            while (times[hi] - times[lo]).total_seconds() > window_seconds:
                ...
                lo += 1
            if hi - lo + 1 >= STRUCTURING_MIN_COUNT and window_sum > best_sum:
