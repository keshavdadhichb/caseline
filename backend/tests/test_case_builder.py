...
def _pipeline():
    ...
    return filtered, rule_flags, graph_flags, records


def test_case_builder_ring_aggregator_has_full_evidence():
    filtered, rule_flags, graph_flags, records = _pipeline()
    record = next(r for r in records if r.account_id == "4521")

    case = case_builder(record, rule_flags, graph_flags, filtered)
...
def test_case_builder_low_risk_account_has_no_ring():
    ...
    case = case_builder(low_record, rule_flags, graph_flags, filtered)
