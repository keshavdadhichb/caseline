...
function ThresholdChart({ file }: { file: CaseFile }) {
  ...
  return (
    <div style={{ animation: "fadeUp6 300ms var(--ease) both", animationDelay: "80ms" }}>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: "auto", display: "block" }} role="img"
        ...
        <line x1={PAD_L} y1={BASE} x2={W - PAD_R} y2={BASE} stroke="var(--border)" strokeWidth="1" />
        <g style={{ transformOrigin: `${PAD_L}px ${thresholdY}px`, animation: "strike 500ms var(--ease) both" }}>
          <line x1={PAD_L} y1={thresholdY} x2={W - PAD_R} y2={thresholdY} stroke="var(--risk-high-dot)" strokeWidth="1" strokeDasharray="4 4" />
        </g>
        <text x={PAD_L} y={thresholdY - 8} fontFamily="var(--mono)" fontSize="11" fill="var(--risk-high-fg)">{usd(THRESHOLD, 0)}</text>
        ...
        {deposits.map((d, i) => {
          ...
          return (
            <circle key={d.txn_id + i} cx={x(new Date(d.ts).getTime())} cy={y(d.amount)}
              r={isNear ? 3.5 : 2} fill={isAnnotated ? "var(--risk-high-dot)" : isNear ? "var(--accent)" : "var(--border)"}
              style={{ transformBox: "fill-box", transformOrigin: "center", animation: "dotIn 300ms var(--ease) both", animationDelay: `${0.45 + i * 0.02}s` }}>
        ...
        {annotate && (
          <g style={{ animation: "fadeIn 300ms ease-out both", animationDelay: "1.1s" }}>
            ...
            <text x={Math.min(x(new Date(annotate.ts).getTime()) + 26, W - 116)} y={y(annotate.amount) + 41}
              fontFamily="var(--mono)" fontSize="12" fill="var(--risk-high-fg)">{usd(annotate.amount)}</text>
            <text x={Math.min(x(new Date(annotate.ts).getTime()) + 26, W - 116)} y={y(annotate.amount) + 55}
              fontFamily="var(--sans)" fontSize="10.5" fill="var(--muted)">
              {pct((THRESHOLD - annotate.amount) / THRESHOLD)} under threshold
            </text>
          </g>
        )}

        <text x={PAD_L} y={200} fontFamily="var(--mono)" fontSize="11" fill="var(--faint)">{fmtDay(tMin)}</text>
        <text x={W - PAD_R} y={200} textAnchor="end" fontFamily="var(--mono)" fontSize="11" fill="var(--faint)">{fmtDay(tMax)}</text>
      </svg>
      <div className="label" style={{ display: "flex", gap: 16, marginTop: 10, flexWrap: "wrap", marginBottom: 0 }}>
        <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}><span className="dot" style={{ background: "var(--accent)" }} />Near-threshold deposits</span>
        <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}><span className="dot" style={{ background: "var(--border)" }} />Other activity</span>
        <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}><span style={{ width: 12, borderTop: "1px dashed var(--risk-high-dot)" }} />Reporting threshold</span>
      </div>
    </div>
  );
}

function StatTile({ label, value, breakdown }: { label: string; value: string; breakdown: [string, string][] }) {
  ...
  return (
    <div style={{ background: "var(--tint)", borderRadius: 14, alignSelf: "start" }}>
      <button onClick={() => setOpen((o) => !o)} aria-expanded={open} className="hv-tint"
        ...
        <span className="label" style={{ color: "var(--muted)", marginBottom: 0 }}>{label}</span>
...
function CasePanel({ file, stats, onDraftSar, onOpenFlow }: {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      <div style={{ animation: "fadeUp6 300ms var(--ease) both" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, animation: "fadeUp6 300ms var(--ease) both", animationDelay: "40ms" }}>
        <StatTile label="Risk score" value={file.score.toFixed(2)}
          breakdown={[["Tier", file.risk_level], ["Signals", String(file.typologies.length)], ["Action", file.recommended_action]]} />
        <StatTile label="Inbound" value={compactUsd(inSum)}
          breakdown={[["Transactions", num(inbound.length)], ["Largest", inbound.length ? usd(Math.max(...inbound.map((r) => r.amount))) : "—"], ["Total", usd(inSum)]]} />
        <StatTile label="Outbound" value={compactUsd(outSum)}
          breakdown={[["Transactions", num(outbound.length)], ["Largest", outbound.length ? usd(Math.max(...outbound.map((r) => r.amount))) : "—"], ["Total", usd(outSum)]]} />
      </div>

      <ThresholdChart file={file} />

      <div style={{ animation: "fadeUp6 300ms var(--ease) both", animationDelay: "120ms" }}>
        <SectionLabel style={{ marginBottom: 4 }}>Evidence</SectionLabel>
        {file.evidence.map((ev, i) => <EvidenceRow key={i} ev={ev} />)}
      </div>

      <div style={{ animation: "fadeUp6 300ms var(--ease) both", animationDelay: "160ms" }}>
        ...
        <button onClick={() => setWhyOpen((o) => !o)} aria-expanded={whyOpen} className="hv-row"
          style={{ display: "flex", alignItems: "center", gap: 10, width: "100%", marginTop: 12, padding: "10px 0", borderTop: "1px solid var(--border)" }}>
          <span className="mono" style={{ fontSize: 13, color: "var(--muted)" }}>risk score {file.score.toFixed(2)} · {file.risk_level}</span>
          <span style={{ flex: 1 }} />
          <Chevron open={whyOpen} />
        </button>
        <Collapse open={whyOpen}>
          <div style={{ padding: "2px 0 8px", display: "flex", flexDirection: "column", gap: 6 }}>
            <div className="mono" style={{ fontSize: 12.5, lineHeight: 1.5, color: "var(--muted)" }}>{file.explanation}</div>
            {stats && <div className="mono" style={{ fontSize: 12.5, lineHeight: 1.5, color: "var(--muted)" }}>{stats.scoring_formula}</div>}
          </div>
        </Collapse>
      </div>

      <div style={{ background: "var(--tint)", borderRadius: 14, padding: 16, display: "flex", flexDirection: "column", gap: 12, animation: "fadeUp6 300ms var(--ease) both", animationDelay: "200ms" }}>
        <Pill bg={t.bg} fg={t.fg} dot={t.dot}>Recommended: {file.recommended_action}</Pill>
        <p style={{ margin: 0, fontSize: 13.5, lineHeight: 1.5, color: "var(--muted)", maxWidth: "52ch" }}>{file.explanation}</p>
...
function FlowPanel({ file }: { file: CaseFile }) {
  ...
  if (!ring) return <div style={{ color: "var(--muted)", fontSize: 13.5 }}>No ring subgraph on this case.</div>;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ animation: "fadeUp6 300ms var(--ease) both" }}>
        <Pill bg="var(--risk-high-bg)" fg="var(--risk-high-fg)" dot="var(--risk-high-dot)">
          Consolidation ring · {senders.length} senders
        </Pill>
      </div>

      <div style={{ position: "relative", animation: "fadeUp6 300ms var(--ease) both", animationDelay: "40ms" }}>
        {isolate && (
          <div className="mono" style={{ position: "absolute", top: 10, left: 10, background: "var(--surface)", border: "1px solid var(--border-strong)", borderRadius: 10, padding: "10px 12px", boxShadow: "0 6px 20px rgba(73,77,95,.10)", fontSize: 12.5, lineHeight: 1.5, color: "var(--ink)", maxWidth: 230, zIndex: 2, animation: "fadeUp6 300ms var(--ease) both" }}>
        ...
        <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: "auto", display: "block" }} role="img"
          ...
          <g fill="none" strokeWidth="1.5">
            {senderPos.map((p, i) => (
              <path key={`e${i}`} d={`M${p.x},${p.y} Q${(p.x + CX) / 2},${(p.y + CY) / 2 - 18} ${CX - 24},${CY}`}
                stroke={isolate ? "var(--risk-high-dot)" : "var(--border-strong)"} pathLength={1}
          ...
          <g>
            {others.map((n, i) => (
              <circle key={n} cx={150 + i * 40} cy={26} r={7} fill="var(--border)"
                style={{ opacity: isolate ? 0.15 : 1, transition: "opacity 300ms", transformBox: "fill-box", transformOrigin: "center", animation: "nodeIn 300ms var(--ease) both" }} />
            ))}
            {senderPos.map((p, i) => (
              <circle key={senders[i]} cx={p.x} cy={p.y} r={9} fill="var(--border-strong)"
                style={{ transformBox: "fill-box", transformOrigin: "center", animation: "nodeIn 300ms var(--ease) both", animationDelay: `${0.05 + i * 0.025}s` }}>
                <title>{senders[i]}</title>
              </circle>
            ))}
            <circle cx={CX} cy={CY} r={22} fill="var(--accent)" stroke={isolate ? "var(--risk-high-dot)" : "transparent"} strokeWidth="3"
              onClick={() => setIsolate((v) => !v)}
              style={{ cursor: "pointer", transformBox: "fill-box", transformOrigin: "center", animation: "nodeIn 300ms var(--ease) both", animationDelay: ".3s", transition: "stroke 300ms" }}>
              <title>{hub}</title>
            </circle>
            {exitPos.map((p, i) => (
              <circle key={exits[i]} cx={p.x} cy={p.y} r={11} fill="var(--risk-high-dot)"
                style={{ transformBox: "fill-box", transformOrigin: "center", animation: "nodeIn 300ms var(--ease) both", animationDelay: `${0.55 + i * 0.05}s` }}>
                <title>{exits[i]}</title>
              </circle>
            ))}
          </g>
          <g fontFamily="var(--mono)" fontSize="10" fill="var(--muted)" style={{ animation: "fadeIn 300ms ease-out both", animationDelay: ".7s" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, animation: "fadeUp6 300ms var(--ease) both", animationDelay: "80ms" }}>
        <Button variant="outline" onClick={() => setIsolate((v) => !v)} aria-pressed={isolate}>
          {isolate ? "Show all activity" : "Isolate the ring"}
        </Button>
        <span style={{ fontSize: 12.5, color: "var(--faint)" }}>Selecting the violet node isolates the ring.</span>
      </div>

      <div style={{ borderTop: "1px solid var(--border)", paddingTop: 14, display: "flex", flexDirection: "column", gap: 8, animation: "fadeUp6 300ms var(--ease) both", animationDelay: "120ms" }}>
...
function TablePanel({ results, cases, onOpenCase }: {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column" }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 100px 86px 16px", gap: 8, padding: "0 2px 8px", borderBottom: "1px solid var(--border-strong)", animation: "fadeUp6 300ms var(--ease) both" }}>
      ...
      {rows.map((r) => {
        ...
        return (
          <div key={r.account_id} style={{ borderBottom: "1px solid var(--border)", animation: "fadeUp6 300ms var(--ease) both" }}>
            <button onClick={() => setOpenId(open ? null : r.account_id)} aria-expanded={open} className="hv-row"
              ...
              <span style={{ display: "block", marginTop: 3, fontSize: 13, lineHeight: 1.5, color: "var(--faint)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.explanation}</span>
            </button>
            <Collapse open={open}>
              <div style={{ padding: "4px 2px 16px", display: "flex", flexDirection: "column", gap: 10 }}>
                <p style={{ margin: 0, fontSize: 13.5, lineHeight: 1.5, color: "var(--muted)", maxWidth: "52ch" }}>{r.explanation}</p>
      ...
      <div style={{ padding: "12px 2px 0", fontSize: 12.5, color: "var(--faint)" }}>
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
      ...
      <div style={{ flex: 1, height: 6, borderRadius: 999, background: "var(--tint)" }}>
        <div style={{ width: `${max ? Math.max(2, (value / max) * 100) : 0}%`, height: 6, borderRadius: 999, background: accent ? "var(--accent)" : "var(--border-strong)" }} />
      </div>
      <span className="mono" style={{ width: 56, textAlign: "right", fontSize: 13, color: accent ? "var(--ink)" : "var(--muted)" }}>
...
function MethodPanel({ method }: { method: MethodResponse }) {
  ...
  const cell = (v: string, strong = false) => (
    <div className="mono" style={{ padding: "9px 0", borderBottom: "1px solid var(--border)", textAlign: "right", fontSize: 13, color: strong ? "var(--ink)" : "var(--muted)" }}>{v}</div>
  );
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      <div style={{ animation: "fadeUp6 300ms var(--ease) both" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 86px 86px", fontSize: 13.5 }}>
          ...
          <div className="label" style={{ padding: "8px 0", borderBottom: "1px solid var(--border)", textAlign: "right", marginBottom: 0 }}>Baseline</div>
          <div className="label" style={{ padding: "8px 0", borderBottom: "1px solid var(--border)", textAlign: "right", marginBottom: 0 }}>Caseline</div>

          <div style={{ padding: "9px 0", borderBottom: "1px solid var(--border)", color: "var(--ink)" }}>Flags raised</div>
          {cell(num(b.flags))}{cell(num(c.flags), true)}
          <div style={{ padding: "9px 0", borderBottom: "1px solid var(--border)", color: "var(--ink)" }}>False-positive rate</div>
          {cell(pct(b.fpr, 2))}{cell(pct(c.fpr, 2), true)}
          <div style={{ padding: "9px 0", borderBottom: "1px solid var(--border)", color: "var(--ink)" }}>Precision</div>
          {cell(pct(b.precision))}{cell(pct(c.precision), true)}
          <div style={{ padding: "9px 0", borderBottom: "1px solid var(--border)", color: "var(--ink)" }}>Recall</div>
          {cell(pct(b.recall))}{cell(pct(c.recall), true)}
        </div>
      </div>

      <div style={{ display: "flex", flexDirection: "column", gap: 14, animation: "fadeUp6 300ms var(--ease) both", animationDelay: "40ms" }}>
        <div>
          <div style={{ fontSize: 12.5, color: "var(--muted)", marginBottom: 8 }}>Flags raised on the same window</div>
        ...
        <div>
          <div style={{ fontSize: 12.5, color: "var(--muted)", marginBottom: 8 }}>False-positive rate</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      ...
      {method.patterns && (
        <div style={{ animation: "fadeUp6 300ms var(--ease) both", animationDelay: "160ms" }}>
          ...
          <p style={{ margin: "0 0 10px", fontSize: 13.5, lineHeight: 1.55, color: "var(--muted)", maxWidth: "56ch" }}>
      ...
      <div style={{ animation: "fadeUp6 300ms var(--ease) both", animationDelay: "200ms" }}>
        <Pill bg="var(--risk-ok-bg)" fg="var(--risk-ok-fg)" dot="var(--risk-ok-dot)">
...
function SarPanel({ file }: { file: CaseFile }) {
  ...
  return (
    <div style={{ padding: "12px 8px 8px", display: "flex", flexDirection: "column", gap: 24, maxWidth: 560 }}>
      <div className="mono" style={{ display: "flex", flexDirection: "column", gap: 4, fontSize: 13, lineHeight: 1.5, color: "var(--muted)", animation: "fadeUp6 300ms var(--ease) both" }}>
        <span>Case reference · {file.case_id}</span>
        <span>{new Date().toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}</span>
        <span>Prepared by Caseline agent</span>
      </div>
      <div style={{ borderTop: "1px solid var(--border)" }} />
      <div style={{ display: "flex", flexDirection: "column", gap: 18, animation: "fadeUp6 300ms var(--ease) both", animationDelay: "40ms" }}>
        ...
        {narrative
          ...
          : <p style={{ margin: 0, fontSize: 15, lineHeight: 1.7, color: "var(--muted)" }}>Drafting…</p>}
      </div>
      <div style={{ display: "flex", gap: 8, paddingTop: 8, animation: "fadeUp6 300ms var(--ease) both", animationDelay: "80ms" }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 28, maxWidth: 560, padding: "4px 2px" }}>
      <div style={{ animation: "fadeUp6 300ms var(--ease) both" }}>
        ...
        <p style={{ margin: "0 0 12px", fontSize: 13.5, lineHeight: 1.62, color: "var(--muted)", maxWidth: "56ch" }}>
      ...
      <div style={{ animation: "fadeUp6 300ms var(--ease) both", animationDelay: "40ms" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {[
            ...
            <div key={i} style={{ display: "flex", gap: 12 }}>
              <span className="mono" style={{ fontSize: 13, color: "var(--faint)", width: 20, flex: "none", paddingTop: 1 }}>{String(i + 1).padStart(2, "0")}</span>
...
export function Canvas({
  ...
  return (
    <aside aria-label="Canvas" style={{
      ...
      animation: "canvasIn 420ms var(--ease) both", transition: "width 420ms var(--ease)",
    }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "18px 20px", borderBottom: "1px solid var(--border)" }}>
        ...
        {sub && <span className="mono" style={{ fontSize: 13, color: "var(--muted)" }}>{sub}</span>}
        <span style={{ flex: 1 }} />
        <button onClick={onWide} className="hv-tint" style={{ fontSize: 13.5, color: "var(--muted)", padding: "4px 10px", borderRadius: 10 }}>
          {wide ? "Narrow" : "Widen"}
        </button>
      ...
      <div className="scroll" style={{ flex: 1, minHeight: 0, padding: 20, display: "flex", flexDirection: "column", gap: 24 }}>
        ...
        {mode.kind === "method" && (method ? <MethodPanel method={method} /> : <span style={{ color: "var(--muted)", fontSize: 13.5 }}>Loading metrics…</span>)}
        {mode.kind === "sar" && caseFile && <SarPanel file={caseFile} />}
        {mode.kind === "about" && (stats ? <AboutPanel stats={stats} method={method} /> : <span style={{ color: "var(--muted)", fontSize: 13.5 }}>Loading…</span>)}
      </div>

      <div className="label" style={{ borderTop: "1px solid var(--border)", padding: "12px 20px", lineHeight: 1.5, marginBottom: 0 }}>
        {footer}
      </div>
    </aside>
  );
